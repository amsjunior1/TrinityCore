...
void BattlefieldWG::OnBattleEnd(bool endByTimer)
{
    ...
    for (GuidSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin(); itr != m_PlayersInWar[GetDefenderTeam()].end(); ++itr)
    {
        if (Player* player = ObjectAccessor::FindPlayer(*itr))
        {
            ...
            else if (!GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_DEF))
            ...
            else if (GetData(BATTLEFIELD_WG_DATA_DAMAGED_TOWER_ATT))
    ...
    for (GuidSet::const_iterator itr = m_PlayersInWar[GetAttackerTeam()].begin(); itr != m_PlayersInWar[GetAttackerTeam()].end(); ++itr)
    {
        if (Player* player = ObjectAccessor::FindPlayer(*itr))
        {
            ...
            if (GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_DEF))
                player->CastSpell(player, SPELL_DESTROYED_TOWER, true);
            else if (GetData(BATTLEFIELD_WG_DATA_DAMAGED_TOWER_DEF))
                player->CastSpell(player, SPELL_DAMAGED_TOWER, true);
        }
    }
